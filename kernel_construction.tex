...
\begin{algorithmic}[1]
\For{each error correction cycle}
    \State measure qubits start with a state $s=-1$ and a change vector $v=1$.
    \If{the window of triplets contains identical measurements}    
        \State $s$ and $v$ do not change
    \Else
        \If{the first and third measurements are the same, and the second is not} 
        ...
        \EndIf
    \EndIf
    \If{the state $s$ reaches a boundary}
